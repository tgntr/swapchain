...
func (k Keeper) SetQueryRequest(ctx sdk.Context, packetSequence uint64, req types.OsmosisQuerySpotPriceRequest) {
...
func (k Keeper) GetQueryRequest(ctx sdk.Context, packetSequence uint64) (types.OsmosisQuerySpotPriceRequest, error) {
	bz := ctx.KVStore(k.storeKey).Get(types.QueryRequestStoreKey(packetSequence))
	if bz == nil {
		return types.OsmosisQuerySpotPriceRequest{}, sdkerrors.Wrapf(types.ErrSample,
			"GetQueryRequest: Result for packet sequence %d is not available.", packetSequence,
		)
	}
	var req types.OsmosisQuerySpotPriceRequest
	k.cdc.MustUnmarshal(bz, &req)
	return req, nil
}

// SetQueryResponse saves the query response
func (k Keeper) SetQueryResponse(ctx sdk.Context, packetSequence uint64, resp types.OsmosisQuerySpotPriceResponse) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetQueryResponse(ctx sdk.Context, packetSequence uint64) (types.OsmosisQuerySpotPriceResponse, error) {
	bz := ctx.KVStore(k.storeKey).Get(types.QueryResponseStoreKey(packetSequence))
	if bz == nil {
		return types.OsmosisQuerySpotPriceResponse{}, sdkerrors.Wrapf(types.ErrSample,
			"GetQueryResponse: Result for packet sequence %d is not available.", packetSequence,
		)
	}
	var resp types.OsmosisQuerySpotPriceResponse
